...
export class VisualizeComponent implements OnInit {
  ...
  topTenActors() {
    ...
    object["fields"]=body;
    this.apiService.topActors(body).subscribe(res => {
      topTenActors = res;

      for(var i = 0; i < topTenActors.length; i++){

        var singleObj = topTenActors[i];
      ...
      this.actorsChart.push (new Chart('actorsCanvas', {
        type: 'bar',
        data: {
  ...
  toneChange() {
    ...
    object["fields"]=body;
    this.apiService.toneChange(body).subscribe(res => {
      toneObj = res;
      ...
      this.toneChart.push (new Chart('toneCanvas', {
        type: 'line',
        data: {
          labels: this.months,
          datasets: [
  ...
  async topTenEventLocation() {
    ...
    var topTenEvents;
      ...
      object["fields"]=body;
      this.apiService.topTenEventLocation(body).subscribe(res => {
        topTenEvents = res;

      for(var i = 0; i < topTenEvents.length; i++ ){

        var countryCode = topTenEvents[i];
        countryCode["name"]=Fips[countryCode["ActionGeo_CountryCode"]];
        var countryName = countryCode["name"];
        var data = topTenEvents[i]["matches"];
        this.topTenEventLocNames.push(countryName);
        this.topTenEventLocData.push(data);

      }

      this.eventChart.push (new Chart('eventCanvas', {
        type: 'bar',
        data: {
  ...
  topTenCameoCodes() {
    ...
    this.apiService.topTenEventLocation(body).subscribe(res => {
      ...
      for(var i = 0; i < 10; i++ ){
        ...
        var obj = topTenCameo[i];
        var obj2 = {};
        obj2["description"] = CAMEO[topTenCameo[i]["EventCode"]];
      ...
      this.cameoChart.push (new Chart('cameoCanvas', {
        type: 'bar',
        data: {
  ...
  numOfArticles() {
    ...
    object["fields"]=body;
    this.apiService.numOfArticles(body).subscribe(res => {
      articlesObj = res;
      ...
      this.articleChart.push (new Chart('canvas', {
        type: 'line',
        data: {
          labels: this.months,
          datasets: [
